/*This code will create a database called 'VWRPrduction', it will contain relational tables with primary and foreign key constraints. Additionally, 
these tables will be assiciated with schemas based on the type of information within the tables.*/


-- This section creates the database.
USE master;

-- Drop database if one exists with the database name I require.
IF DB_ID('VWRProduction') IS NOT NULL  
DROP DATABASE VWRProduction;
...
--Here I create the 3 schemas that will reside within the database.
...
/*This section will create the database tables, the primary keys, foreign keys, constraints and indexes.*/


-- Create table HR.Associates.

/*This table will have a primary key assigned to the "AssociateID" column, as well as a foreign key "ManagerID" assigned to the same table.
Like most tables within the database, the table contains nonclustered indexes which can assist in query performance.*/
IF OBJECT_ID('HR.Associates') IS NOT NULL
DROP TABLE HR.Associates;
...
(
  ...
  CONSTRAINT FK_Associates_Associates FOREIGN KEY(ManagerID) REFERENCES HR.Associates(AssociateID),
  CONSTRAINT CHEK_BirthDate CHECK(BirthDate <= CURRENT_TIMESTAMP)
);

CREATE NONCLUSTERED INDEX idx_nc_LastName   ON HR.Associates(LastName);
CREATE NONCLUSTERED INDEX idx_nc_ZipCode ON HR.Associates(ZipCode);

-- Create table Production.ProductSuppliers.

/*This table's primary key will be assigned to the "ProductSuppliersID" column. The nonclustered indexes will be applied to the 
'NameOfCompany' and 'ZipCode' columns.*/
IF OBJECT_ID('Production.ProductSuppliers') IS NOT NULL
DROP TABLE Production.ProductSuppliers;


CREATE TABLE Production.ProductSuppliers
(
  ProductSupplierID INT  NOT NULL IDENTITY,
  NameOfCompany   NVARCHAR(40) NOT NULL,
...
CREATE NONCLUSTERED INDEX idx_nc_NameOfCompany ON Production.ProductSuppliers(NameOfCompany);
CREATE NONCLUSTERED INDEX idx_nc_postalcode  ON Production.ProductSuppliers(ZipCode);

-- Create Production.ProductCategories table.

/*This table's primary key will be assigned to the "ProductCategoryID" column and an index on the 'CategoryName' column.*/
IF OBJECT_ID('Production.ProductCategories') IS NOT NULL
DROP TABLE Production.ProductCateogries;
...
CREATE INDEX categoryname ON Production.ProductCategories(CategoryName);

-- Create Production.ProductDetails table.

/*This table will contain one primary key, two foreign keys and a check constraint. The primary key will be assigned to the "ProductID" column. The two foreign
keys will point to two differen tables, the first will point to the ProductCategories tables by way of the ProductCategoryID column, the other will point
to the ProductSuppliers table by way of the "ProductSupplierID" column. Finally, the check constraint which assures the amount entered for the "UnitPrice"
is equal to or greater than zero.*/

IF OBJECT_ID('Production.ProductDetails') IS NOT NULL
DROP TABLE Production.ProductDetails;
...
(
  ...
  UnitPrice    MONEY  NOT NULL CONSTRAINT DFLT_ProductDetails_UnitPrice DEFAULT(0),
  Discontinued        BIT NOT NULL CONSTRAINT DFLT_ProductDetails_Discontinued DEFAULT(0),
  CONSTRAINT PK_ProductDetails PRIMARY KEY(ProductID),
  CONSTRAINT FK_ProductDetails_ProductCategories FOREIGN KEY(ProductCategoryID) REFERENCES Production.ProductCategories(ProductCategoryID),
  CONSTRAINT FK_ProductDetails_ProductSuppliers FOREIGN KEY(ProductSupplierID) REFERENCES Production.ProductSuppliers(ProductSupplierID),
  CONSTRAINT CHEK_ProductDetails_UnitPrice CHECK(UnitPrice >= 0)
);

/*These indexes will assist in enhanced retreivel of data associated with the three columns that make up the nonclustered objects.*/
CREATE NONCLUSTERED INDEX idx_nc_categoryid  ON Production.ProductDetails(ProductCategoryID);
CREATE NONCLUSTERED INDEX idx_nc_productname ON Production.ProductDetails(ProductName);
CREATE NONCLUSTERED INDEX idx_nc_supplierid  ON Production.ProductDetails(ProductSupplierID);

-- Create Sales.Customers table.

/*This table will contain a primary key associated with the "CustomerID" column.*/
IF OBJECT_ID('Sales.Customers') IS NOT NULL
...
(
  ...
  NameOfCompany  NVARCHAR(40) NOT NULL,
  ContactName    NVARCHAR(30) NOT NULL,
  TitleOfContact NVARCHAR(30) NOT NULL,
  Address        NVARCHAR(60) NOT NULL,
  City           NVARCHAR(15) NOT NULL,
  Region         NVARCHAR(15) NULL,
  ZipCode        NVARCHAR(10) NULL,
...
CREATE NONCLUSTERED INDEX idx_nc_NameOfCompany ON Sales.Customers(NameOfCompany);
CREATE NONCLUSTERED INDEX idx_nc_zipcode  ON Sales.Customers(ZipCode);
CREATE NONCLUSTERED INDEX idx_nc_region      ON Sales.Customers(Region);

-- Create Sales.ProductShippers table.

/*This table will contain one primary key associated with the "ProductShipperID" column.*/
IF OBJECT_ID('Sales.ProductShipers') IS NOT NULL
DROP TABLE Sales.ProductShipppers;


CREATE TABLE Sales.ProductShippers
(
  ProductShipperID  INT NOT NULL IDENTITY,
  NameOfCompany     NVARCHAR(40) NOT NULL,
  Phone             NVARCHAR(24) NOT NULL,
  CONSTRAINT PK_ProductShippers PRIMARY KEY(ProductShipperID)
);

-- Create Sales.CustomerOrders table.

/*This table will contain one primary key and three foreign keys, first, the primary key will be assigned to the "OrderID" column. The three foreign keys will 
be "CustomerID" which will point to the same column name on the "Customers" table, the "AssociateID" column which will point to the same column on the 
"Associates" table. Finally, the "ProductShipperID" column will point to the "ProductShipperID" column on the "ProductShippers" table.*/
IF OBJECT_ID('Sales.CustomerOrders') IS NOT NULL 
...
(
  ...
  ProductShipperID  INT          NOT NULL,
  Freight           MONEY        NOT NULL,
  ShipName          NVARCHAR(30) NOT NULL,
  ShipAddress       NVARCHAR(40) NOT NULL,  
  ShipCity          NVARCHAR(15) NOT NULL,
  ShipRegion	    NVARCHAR(20) NULL,
  ShipZipCode       NVARCHAR(15) NULL,
  ShipCountry       NVARCHAR(15) NOT NULL,
  CONSTRAINT PK_CustomerOrders PRIMARY KEY(OrderID),
  CONSTRAINT FK_CustomerOrders_Customers FOREIGN KEY(CustomerID) REFERENCES Sales.Customers(CustomerID), 
  CONSTRAINT FK_CustomerOrders_Associates FOREIGN KEY(AssociateID) REFERENCES HR.Associates(AssociateID),
  CONSTRAINT FK_CustomerOrders_ProductShippers FOREIGN KEY(ProductShipperID) REFERENCES Sales.ProductShippers(ProductShipperID)
);

/*These nonclustered indexes will enhance data retreivel on the specified column within the "CustomerOrders" table this is due to the pointers 
to the physical rows.*/

CREATE NONCLUSTERED INDEX idx_nc_customerid     ON Sales.CustomerOrders(CustomerID);
CREATE NONCLUSTERED INDEX idx_nc_associteid     ON Sales.CustomerOrders(AssociateID);
CREATE NONCLUSTERED INDEX idx_nc_shipperid      ON Sales.CustomerOrders(ProductShipperID);
CREATE NONCLUSTERED INDEX idx_nc_orderdate      ON Sales.CustomerOrders(OrderDate);
CREATE NONCLUSTERED INDEX idx_nc_shippeddate    ON Sales.CustomerOrders(ShippedDate);
CREATE NONCLUSTERED INDEX idx_nc_shipzipcode ON Sales.CustomerOrders(shipzipcode);

-- Create table Sales.CustomerOrderDetails.

/*The Sales.CustomerOrderDetails table primary key will be 'Composite' which means there will be more than one column assigned to primary status. The two columns 
in this case wil be "OrderID" and "ProductID". The foreign keys will be assigned to the "OrderID" and "ProductID" columns, the first will point to the "CustomerOrders" table
the the other will point to the "ProductDetails" table. The three check constraints will assure the "Discount" range is between 0 and 1, "Qty" will be above 0. Finally,
the constraint on the  "UnitPrice" column will ensure the values placed will be greater than or equal to 0.*/

IF OBJECT_ID('Sales.CustomerOrderDetails') IS NOT NULL
DROP TABLE Sales.CustomerOrderDetails;
...
(
  ...
  Qty        SMALLINT NOT NULL CONSTRAINT DFLT_CustomerOrderDetails_Qty DEFAULT(1),
  Discount   NUMERIC(4, 3) NOT NULL CONSTRAINT DFLT_CustomerOrderDetails_Discount DEFAULT(0),
  CONSTRAINT PK_CustomerOrderDetails PRIMARY KEY(OrderID, ProductID),
  CONSTRAINT FK_CustomerOrderDetails_CustomerOrders FOREIGN KEY(OrderID)REFERENCES Sales.CustomerOrders(OrderID),
  CONSTRAINT FK_CustomerOrderDetails_ProductDetails FOREIGN KEY(ProductID) REFERENCES Production.ProductDetails(ProductID),
  CONSTRAINT CHK_Discount  CHECK (Discount BETWEEN 0 AND 1),
  CONSTRAINT CHK_Qty  CHECK (Qty > 0),
  CONSTRAINT CHK_UnitPrice CHECK (UnitPrice >= 0)
);

CREATE NONCLUSTERED INDEX idx_nc_orderid   ON Sales.CustomerOrderDetails(OrderID);
CREATE NONCLUSTERED INDEX idx_nc_productid ON Sales.CustomerOrderDetails(ProductID);


/*The code below will populate the tables by using an the INSERT statement*/

--HR.Associates data population.
SET IDENTITY_INSERT HR.Associates ON;
INSERT INTO HR.Associates(AssociateID, LastName, FirstName, JobTitle, CourtesyTitle, BirthDate, HireDate, Address, City, Region, ZipCode, Country, Phone, ManagerID)
...
SET IDENTITY_INSERT HR.Associates OFF;

--Production.Suppliers population.
SET IDENTITY_INSERT Production.ProductSuppliers ON;
INSERT INTO Production.ProductSuppliers(ProductSupplierID, NameOfCompany, ContactName, JobTitle, Address, City, Region, ZipCode, Country, Phone, Fax)
...
SET IDENTITY_INSERT Production.ProductSuppliers OFF;


--Populate the Production.ProductCategories Table.
...
--This code will populate the Production.ProductDetials table.
SET IDENTITY_INSERT Production.ProductDetails ON;
INSERT INTO Production.ProductDetails(ProductID, ProductName, ProductSupplierID, ProductCategoryID, UnitPrice, Discontinued)
...
SET IDENTITY_INSERT Production.ProductDetails OFF;

--Sales.Customers table population.
SET IDENTITY_INSERT Sales.Customers ON;
INSERT INTO Sales.Customers(CustomerID, NameOfCompany, ContactName, TitleOfContact, Address, City, Region, ZipCode, Country, Phone, Fax)
...
--This code populates the Sales.ProductShippers table.
...
--Population of the Sales.CustomerOrders table. 
SET IDENTITY_INSERT Sales.CustomerOrders ON;
INSERT INTO Sales.CustomerOrders(OrderID, CustomerID, AssociateID, OrderDate, RequiredDate, ShippedDate, ProductShipperID, Freight, ShipName, ShipAddress, ShipCity, ShipRegion, ShipZipCode, ShipCountry)
...
SET IDENTITY_INSERT Sales.CustomerOrders OFF; 



-- Populate table Sales.CustomerOrderDetails.
INSERT INTO Sales.CustomerOrderDetails(OrderID, ProductID, UnitPrice, Qty, Discount)
